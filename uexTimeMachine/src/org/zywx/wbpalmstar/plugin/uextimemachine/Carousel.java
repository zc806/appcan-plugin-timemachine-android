...
public class Carousel extends CarouselSpinner implements
	...
	/**
	 * The axe angle 俯视角度大小
	 */
	private float mTheta = (float) (24.0f * (Math.PI / 180.0));

	/**
	 * If items should be reflected
	 */
	private boolean mUseReflection = true;

	private int touchSlop;

	private float density;

	private float centerYFactor = 0.0f;
	private float translateYFactor = 0.0f;
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		density = dm.density;
		if (density == 2.0) {
			if (dm.heightPixels == 960 && dm.widthPixels == 640) {
				centerYFactor = 0.7f;
				translateYFactor = 0.28f;
			} else {
				centerYFactor = 0.6f;
				translateYFactor = 0.4f;
			}
		} else if (density == 1.5) {
			centerYFactor = 0.6f;
			translateYFactor = 0.33f;
		} else {
			centerYFactor = 0.7f;
			translateYFactor = 0.28f;
		}
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		...
		for (int j = 0; j < childCount; j++) {
			CarouselImageView view = (CarouselImageView) getAdapter().getView(
					j, null, null);
			if (i == 0)
				view.setDrawn(false);
	...
	protected boolean getChildStaticTransformation(View child,
		...
		mCamera.translate(((CarouselImageView) child).getX(),
				((CarouselImageView) child).getY() + child.getHeight()
						* translateYFactor, ((CarouselImageView) child).getZ());
		// Align the item
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
			float velocityY) {
		float absDistance = Math.abs(e2.getX() - e1.getX());
		if (absDistance < touchSlop) {
			return true;
		}
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
			float distanceY) {
		float absDistance = Math.abs(e2.getX() - e1.getX());
		// BDebug.i(TAG, "onScroll()----->absDistance:" + absDistance +
		// " touchSlop:" + touchSlop + " scrolled:"
		// + (absDistance > touchSlop));
		if (absDistance < touchSlop) {
			return true;
		}
		if (localLOGV)
...
//			}
			performItemClick(mDownTouchView, nowPosition,
					mAdapter.getItemId(nowPosition));
	 ...
	 * @param angleOffset
	 */
	private void Calculate3DPosition(CarouselImageView child, int diameter,
			float angleOffset) {
	...
	private void makeAndAddView(int position, float angleOffset) {
		...
		if (!mDataChanged) {
			...
			if (child != null) {
				...
				child = (CarouselImageView) mAdapter.getView(position, null,
						this);
	...
	void scrollToChild(int i) {
		...
		CarouselImageView view = (CarouselImageView) getAdapter().getView(i,
				null, null);
	 ...
	 */
	private void setUpChild(CarouselImageView child, int index,
			float angleOffset) {
		...
		if (mInLayout) {
			h = (getMeasuredHeight() - getPaddingBottom() - getPaddingTop());
		...
		Calculate3DPosition(child, w / 6, angleOffset);
	...
	void trackMotionScroll(float deltaAngle) {
		...
		for (int i = 0; i < getAdapter().getCount(); i++) {
			CarouselImageView child = (CarouselImageView) getAdapter().getView(
					i, null, null);
			...
			if (i == 0) {
				if (angle < 20 || angle > 340) {
					...
					mOnItemSelectedListener.onItemSelected(this,
							child.getRootView(), 0, getAdapter().getItemId(0));
				}
				if (angle > 160 && angle < 200) {
					nowPosition = 1;
					mOnItemSelectedListener.onItemSelected(this,
							child.getRootView(), 1, getAdapter().getItemId(1));
				}
			}
			child.setCurrentAngle(angle);
			Calculate3DPosition(child, getWidth() / 6, angle);
		}
